...
def preprocessing(train_df, test_df):
    ...
    train_df['대출 목적 그룹'] = train_df['대출 목적'].map(category_groups)
    test_df['대출 목적 그룹'] = test_df['대출 목적'].map(category_groups)
    train_df = pd.get_dummies(train_df, columns=['대출 목적 그룹'])
    test_df = pd.get_dummies(test_df, columns=['대출 목적 그룹'])
    train_df.drop(['대출 목적'], axis=1, inplace=True)
    test_df.drop(['대출 목적'], axis=1, inplace=True)
    '''
